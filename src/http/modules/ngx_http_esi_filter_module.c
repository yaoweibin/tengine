...
#define NGX_HTTP_ESI_COND_IF          1
#define NGX_HTTP_ESI_COND_ELSE        2


#define NGX_HTTP_ESI_NO_ENCODING      0
#define NGX_HTTP_ESI_URL_ENCODING     1
#define NGX_HTTP_ESI_ENTITY_ENCODING  2
...
typedef struct {
    ...
    unsigned                  multiple:1;
} ngx_http_esi_param_t;


typedef struct {
    ngx_str_t                 name;
    ngx_http_esi_command_pt   handler;
    ngx_http_esi_param_t     *params;
...
#define NGX_HTTP_ESI_DATE_LEN       2048

#define NGX_HTTP_ESI_ADD_PREFIX     1
#define NGX_HTTP_ESI_ADD_ZERO       2
...
static ngx_int_t ngx_http_esi_parse(ngx_http_request_t *r,
    ngx_http_esi_ctx_t *ctx);
static ngx_str_t *ngx_http_esi_get_variable(ngx_http_request_t *r,
    ngx_str_t *name, ngx_uint_t key);
static ngx_int_t ngx_http_esi_evaluate_string(ngx_http_request_t *r,
    ngx_http_esi_ctx_t *ctx, ngx_str_t *text, ngx_uint_t flags);
static ngx_int_t ngx_http_esi_regex_match(ngx_http_request_t *r,
...
static ngx_int_t ngx_http_esi_stub_output(ngx_http_request_t *r, void *data,
    ngx_int_t rc);
static ngx_int_t ngx_http_esi_set_variable(ngx_http_request_t *r, void *data,
    ngx_int_t rc);
static ngx_int_t ngx_http_esi_echo(ngx_http_request_t *r,
    ngx_http_esi_ctx_t *ctx, ngx_str_t **params);
static ngx_int_t ngx_http_esi_config(ngx_http_request_t *r,
    ngx_http_esi_ctx_t *ctx, ngx_str_t **params);
static ngx_int_t ngx_http_esi_set(ngx_http_request_t *r,
    ngx_http_esi_ctx_t *ctx, ngx_str_t **params);
static ngx_int_t ngx_http_esi_if(ngx_http_request_t *r,
    ngx_http_esi_ctx_t *ctx, ngx_str_t **params);
static ngx_int_t ngx_http_esi_else(ngx_http_request_t *r,
    ngx_http_esi_ctx_t *ctx, ngx_str_t **params);
static ngx_int_t ngx_http_esi_endif(ngx_http_request_t *r,
    ngx_http_esi_ctx_t *ctx, ngx_str_t **params);
static ngx_int_t ngx_http_esi_block(ngx_http_request_t *r,
    ngx_http_esi_ctx_t *ctx, ngx_str_t **params);
static ngx_int_t ngx_http_esi_endblock(ngx_http_request_t *r,
    ngx_http_esi_ctx_t *ctx, ngx_str_t **params);

static ngx_int_t ngx_http_esi_date_gmt_local_variable(ngx_http_request_t *r,
...
static u_char ngx_http_esi_string[] = "<!--";

static ngx_str_t ngx_http_esi_none = ngx_string("(none)");
static ngx_str_t ngx_http_esi_null_string = ngx_null_string;


#define  NGX_HTTP_ESI_INCLUDE_VIRTUAL  0
#define  NGX_HTTP_ESI_INCLUDE_FILE     1
#define  NGX_HTTP_ESI_INCLUDE_WAIT     2
#define  NGX_HTTP_ESI_INCLUDE_SET      3
#define  NGX_HTTP_ESI_INCLUDE_STUB     4

#define  NGX_HTTP_ESI_ECHO_VAR         0
#define  NGX_HTTP_ESI_ECHO_DEFAULT     1
#define  NGX_HTTP_ESI_ECHO_ENCODING    2

#define  NGX_HTTP_ESI_CONFIG_ERRMSG    0
#define  NGX_HTTP_ESI_CONFIG_TIMEFMT   1

#define  NGX_HTTP_ESI_SET_VAR          0
#define  NGX_HTTP_ESI_SET_VALUE        1

#define  NGX_HTTP_ESI_IF_EXPR          0

#define  NGX_HTTP_ESI_BLOCK_NAME       0


static ngx_http_esi_param_t  ngx_http_esi_include_params[] = {
    { ngx_string("virtual"), NGX_HTTP_ESI_INCLUDE_VIRTUAL, 0, 0 },
    { ngx_string("file"), NGX_HTTP_ESI_INCLUDE_FILE, 0, 0 },
    { ngx_string("wait"), NGX_HTTP_ESI_INCLUDE_WAIT, 0, 0 },
    { ngx_string("set"), NGX_HTTP_ESI_INCLUDE_SET, 0, 0 },
    { ngx_string("stub"), NGX_HTTP_ESI_INCLUDE_STUB, 0, 0 },
    { ngx_null_string, 0, 0, 0 }
};


static ngx_http_esi_param_t  ngx_http_esi_echo_params[] = {
    { ngx_string("var"), NGX_HTTP_ESI_ECHO_VAR, 1, 0 },
    { ngx_string("default"), NGX_HTTP_ESI_ECHO_DEFAULT, 0, 0 },
    { ngx_string("encoding"), NGX_HTTP_ESI_ECHO_ENCODING, 0, 0 },
    { ngx_null_string, 0, 0, 0 }
};


static ngx_http_esi_param_t  ngx_http_esi_config_params[] = {
    { ngx_string("errmsg"), NGX_HTTP_ESI_CONFIG_ERRMSG, 0, 0 },
    { ngx_string("timefmt"), NGX_HTTP_ESI_CONFIG_TIMEFMT, 0, 0 },
    { ngx_null_string, 0, 0, 0 }
};


static ngx_http_esi_param_t  ngx_http_esi_set_params[] = {
    { ngx_string("var"), NGX_HTTP_ESI_SET_VAR, 1, 0 },
    { ngx_string("value"), NGX_HTTP_ESI_SET_VALUE, 1, 0 },
    { ngx_null_string, 0, 0, 0 }
};


static ngx_http_esi_param_t  ngx_http_esi_if_params[] = {
    { ngx_string("expr"), NGX_HTTP_ESI_IF_EXPR, 1, 0 },
    { ngx_null_string, 0, 0, 0 }
};


static ngx_http_esi_param_t  ngx_http_esi_block_params[] = {
    { ngx_string("name"), NGX_HTTP_ESI_BLOCK_NAME, 1, 0 },
    { ngx_null_string, 0, 0, 0 }
};


static ngx_http_esi_param_t  ngx_http_esi_no_params[] = {
    { ngx_null_string, 0, 0, 0 }
};
...
static ngx_http_esi_command_t  ngx_http_esi_commands[] = {
    { ngx_string("include"), ngx_http_esi_include,
                       ngx_http_esi_include_params, 0, 0, 1 },
    { ngx_string("echo"), ngx_http_esi_echo,
                       ngx_http_esi_echo_params, 0, 0, 0 },
    { ngx_string("config"), ngx_http_esi_config,
                       ngx_http_esi_config_params, 0, 0, 0 },
    { ngx_string("set"), ngx_http_esi_set, ngx_http_esi_set_params, 0, 0, 0 },

    { ngx_string("if"), ngx_http_esi_if, ngx_http_esi_if_params, 0, 0, 0 },
    { ngx_string("elif"), ngx_http_esi_if, ngx_http_esi_if_params,
                       NGX_HTTP_ESI_COND_IF, 0, 0 },
    { ngx_string("else"), ngx_http_esi_else, ngx_http_esi_no_params,
                       NGX_HTTP_ESI_COND_IF, 0, 0 },
    { ngx_string("endif"), ngx_http_esi_endif, ngx_http_esi_no_params,
                       NGX_HTTP_ESI_COND_ELSE, 0, 0 },

    { ngx_string("block"), ngx_http_esi_block,
                       ngx_http_esi_block_params, 0, 0, 0 },
    { ngx_string("endblock"), ngx_http_esi_endblock,
                       ngx_http_esi_no_params, 0, 1, 0 },

    { ngx_null_string, NULL, NULL, 0, 0, 0 }
};


static ngx_http_variable_t  ngx_http_esi_vars[] = {

    { ngx_string("date_local"), NULL, ngx_http_esi_date_gmt_local_variable, 0,
      NGX_HTTP_VAR_NOCACHEABLE, 0 },

    { ngx_string("date_gmt"), NULL, ngx_http_esi_date_gmt_local_variable, 1,
      NGX_HTTP_VAR_NOCACHEABLE, 0 },
...
ngx_http_esi_header_filter(ngx_http_request_t *r)
{
    ...
    ctx->encoding = NGX_HTTP_ESI_ENTITY_ENCODING;
    ctx->output = 1;
    ...
    ngx_str_set(&ctx->errmsg,
                "[an error occurred while proceesing the directive]");
...
ngx_http_esi_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    ngx_chain_t               *cl, **ll;
    ngx_table_elt_t           *param;
    ngx_http_esi_ctx_t        *ctx, *mctx;
    ngx_http_esi_block_t      *bl;
    ngx_http_esi_param_t      *prm;
    ngx_http_esi_command_t    *cmd;
    ngx_http_esi_loc_conf_t   *slcf;
    ngx_http_esi_main_conf_t  *smcf;
    ngx_str_t                 *params[NGX_HTTP_ESI_MAX_PARAMS + 1];
            ...
            if (rc == NGX_OK) {
                ...
                if (!ctx->output && !cmd->block) {
                    ...
                    if (ctx->block) {
                        ...
                        /* reconstruct the ESI command text */
                ...
                {
                    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                                  "invalid context of ESI command: \"%V\"",
                                  &ctx->command);
                    goto esi_error;
                }
                ...
                ngx_memzero(params,
                           (NGX_HTTP_ESI_MAX_PARAMS + 1) * sizeof(ngx_str_t *));
...
ngx_http_esi_parse(ngx_http_request_t *r, ngx_http_esi_ctx_t *ctx)
{
    ...
    for (p = ctx->pos; p < last; p++) {
        ...
        case esi_comment0_state:
            switch (ch) {
            case '-':
                looked = 3;
                state = esi_comment1_state;
                break;
        ...
        case esi_comment1_state:
            switch (ch) {
            case '-':
                looked = 4;
                state = esi_sharp_state;
                break;
        ...
        case esi_comment_end0_state:
            switch (ch) {
            case '-':
                state = esi_comment_end1_state;
                break;
        ...
        case esi_error_end0_state:
            switch (ch) {
            case '-':
                state = esi_error_end1_state;
                break;
...
static ngx_str_t *
ngx_http_esi_get_variable(ngx_http_request_t *r, ngx_str_t *name,
    ngx_uint_t key)
{
    ngx_uint_t           i;
    ngx_list_part_t     *part;
    ngx_http_esi_var_t  *var;
    ngx_http_esi_ctx_t  *ctx;

    ctx = ngx_http_get_module_ctx(r->main, ngx_http_esi_filter_module);
...
static ngx_int_t
ngx_http_esi_evaluate_string(ngx_http_request_t *r, ngx_http_esi_ctx_t *ctx,
    ngx_str_t *text, ngx_uint_t flags)
{
    ...
    if (n == 0) {
        ...
        if ((flags & NGX_HTTP_ESI_ADD_PREFIX) && text->data[0] != '/') {
    ...
    while (i < text->len) {
        ...
        if (text->data[i] == '$') {
            ...
            val = ngx_http_esi_get_variable(r, &var, key);
    ...
    if (flags & NGX_HTTP_ESI_ADD_PREFIX) {
    ...
    p = ngx_pnalloc(r->pool, len + ((flags & NGX_HTTP_ESI_ADD_ZERO) ? 1 : 0));
    if (p == NULL) {
        return NGX_ERROR;
    }
...
static ngx_int_t
ngx_http_esi_regex_match(ngx_http_request_t *r, ngx_str_t *pattern,
    ngx_str_t *str)
{
#if (NGX_PCRE)
    int                   rc, *captures;
    ...
    if (rgc.named_captures > 0) {
        ...
        for (i = 0; i < (ngx_uint_t) rgc.named_captures; i++, p += size) {
            ...
            vv = ngx_http_esi_get_variable(r, &name, key);
    ...
    ngx_log_error(NGX_LOG_ALERT, r->connection->log, 0,
                  "the using of the regex \"%V\" in ESI requires PCRE library",
                  pattern);
    return NGX_HTTP_ESI_ERROR;
    ...
    ngx_str_t **params)
{
    ...
    ngx_http_esi_var_t          *var;
    ngx_http_esi_ctx_t          *mctx;
    ngx_http_esi_block_t        *bl;
    ngx_http_post_subrequest_t  *psr;

    uri = params[NGX_HTTP_ESI_INCLUDE_VIRTUAL];
    file = params[NGX_HTTP_ESI_INCLUDE_FILE];
    wait = params[NGX_HTTP_ESI_INCLUDE_WAIT];
    set = params[NGX_HTTP_ESI_INCLUDE_SET];
    stub = params[NGX_HTTP_ESI_INCLUDE_STUB];
    ...
    rc = ngx_http_esi_evaluate_string(r, ctx, uri, NGX_HTTP_ESI_ADD_PREFIX);
    ...
    mctx = ngx_http_get_module_ctx(r->main, ngx_http_esi_filter_module);
    ...
    if (set) {
        ...
        psr->handler = ngx_http_esi_set_variable;
        psr->data = ngx_http_esi_get_variable(r, set, key);
        ...
        if (psr->data == NULL) {
            ...
            var->name = *set;
            var->key = key;
            var->value = ngx_http_esi_null_string;
            psr->data = &var->value;
        }
...
ngx_http_esi_stub_output(ngx_http_request_t *r, void *data, ngx_int_t rc)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "esi stub output: \"%V?%V\"", &r->uri, &r->args);
...
static ngx_int_t
ngx_http_esi_set_variable(ngx_http_request_t *r, void *data, ngx_int_t rc)
{
...
static ngx_int_t
ngx_http_esi_echo(ngx_http_request_t *r, ngx_http_esi_ctx_t *ctx,
    ngx_str_t **params)
{
    u_char                     *p;
    uintptr_t                   len;
    ...
    var = params[NGX_HTTP_ESI_ECHO_VAR];
    ...
    value = ngx_http_esi_get_variable(r, var, key);
    ...
    if (value == NULL) {
        value = params[NGX_HTTP_ESI_ECHO_DEFAULT];

        if (value == NULL) {
            value = &ngx_http_esi_none;
    ...
    enc = params[NGX_HTTP_ESI_ECHO_ENCODING];

    if (enc) {
        if (enc->len == 4 && ngx_strncmp(enc->data, "none", 4) == 0) {

            ctx->encoding = NGX_HTTP_ESI_NO_ENCODING;

        } else if (enc->len == 3 && ngx_strncmp(enc->data, "url", 3) == 0) {

            ctx->encoding = NGX_HTTP_ESI_URL_ENCODING;
    ...
    case NGX_HTTP_ESI_URL_ENCODING:
    ...
    case NGX_HTTP_ESI_ENTITY_ENCODING:
    ...
    default: /* NGX_HTTP_ESI_NO_ENCODING */
        len = value->len;
        break;
    }
...
static ngx_int_t
ngx_http_esi_config(ngx_http_request_t *r, ngx_http_esi_ctx_t *ctx,
    ngx_str_t **params)
{
    ngx_str_t  *value;

    value = params[NGX_HTTP_ESI_CONFIG_TIMEFMT];
    ...
    value = params[NGX_HTTP_ESI_CONFIG_ERRMSG];
...
static ngx_int_t
ngx_http_esi_set(ngx_http_request_t *r, ngx_http_esi_ctx_t *ctx,
    ngx_str_t **params)
{
    ngx_int_t            key, rc;
    ...
    name = params[NGX_HTTP_ESI_SET_VAR];
    value = params[NGX_HTTP_ESI_SET_VALUE];

    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "esi set \"%V\" \"%V\"", name, value);

    rc = ngx_http_esi_evaluate_string(r, ctx, value, 0);
...
static ngx_int_t
ngx_http_esi_if(ngx_http_request_t *r, ngx_http_esi_ctx_t *ctx,
    ngx_str_t **params)
{
    u_char       *p, *last;
    ...
    expr = params[NGX_HTTP_ESI_IF_EXPR];
    ...
    rc = ngx_http_esi_evaluate_string(r, ctx, &left, flags);
    ...
    if (p == last) {
        ...
        ctx->conditional = NGX_HTTP_ESI_COND_IF;
    ...
    rc = ngx_http_esi_evaluate_string(r, ctx, &right, flags);
    ...
    if (noregex) {
        ...
        rc = ngx_http_esi_regex_match(r, &right, &left);
...
static ngx_int_t
ngx_http_esi_else(ngx_http_request_t *r, ngx_http_esi_ctx_t *ctx,
    ngx_str_t **params)
{
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "esi else");
    ...
    ctx->conditional = NGX_HTTP_ESI_COND_ELSE;
...
static ngx_int_t
ngx_http_esi_endif(ngx_http_request_t *r, ngx_http_esi_ctx_t *ctx,
    ngx_str_t **params)
{
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "esi endif");
...
static ngx_int_t
ngx_http_esi_block(ngx_http_request_t *r, ngx_http_esi_ctx_t *ctx,
    ngx_str_t **params)
{
    ngx_http_esi_ctx_t    *mctx;
    ngx_http_esi_block_t  *bl;
    ...
    bl->name = *params[NGX_HTTP_ESI_BLOCK_NAME];
    bl->bufs = NULL;
    bl->count = 0;
...
static ngx_int_t
ngx_http_esi_endblock(ngx_http_request_t *r, ngx_http_esi_ctx_t *ctx,
    ngx_str_t **params)
{
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "esi endblock");
...
static ngx_int_t
ngx_http_esi_date_gmt_local_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t gmt)
{
    ngx_http_esi_ctx_t  *ctx;
    ngx_time_t          *tp;
    struct tm            tm;
    char                 buf[NGX_HTTP_ESI_DATE_LEN];
    ...
    v->len = strftime(buf, NGX_HTTP_ESI_DATE_LEN,
...
ngx_http_esi_preconfiguration(ngx_conf_t *cf)
{
    ...
    for (v = ngx_http_esi_vars; v->name.len; v++) {
    ...
    for (cmd = ngx_http_esi_commands; cmd->name.len; cmd++) {
        ...
        if (rc == NGX_BUSY) {
            ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                               "conflicting ESI command \"%V\"", &cmd->name);
        }
